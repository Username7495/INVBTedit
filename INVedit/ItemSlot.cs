	...
	public class ItemSlot : CheckBox
	{
		...
		protected override void OnDragDrop(DragEventArgs e)
		{
			...
			if (e.Effect == DragDropEffects.Link) {
				if (Item == null) {
					Item = new Item(item.ID, (sbyte)(item.Count/2), Slot, item.Damage);
					item.Count -= Item.Count;
				} else {
					sbyte count = (sbyte)Item.Count;
					Item.Count = Math.Min((sbyte)(count+item.Count/2), (sbyte)64);
					item.Count -= (sbyte)(Item.Count-count);
				}
				...
				Item = new Item(Item.ID, (sbyte)count, Slot, Item.Damage);
				other = (over>0) ? new Item(Item.ID, (sbyte)over) : null;
			} else {
		...
		protected override void OnMouseWheel(MouseEventArgs e)
		{
			...
			if ((Control.ModifierKeys & Keys.Shift) == Keys.Shift) {
				...
				Item.Count = (sbyte)Math.Min(Math.Max((int)Item.Count + count, 1), Item.Stack);
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (Item.Count != 1) {
				...
				Color color1 = Color.Black;
				Color color2 = Color.White;
				if (Item.Count > Item.Stack || Item.Count < 1)
