...
namespace INVedit
{
	internal static class Data
	{
		...
		{
			ResourceManager resources = new ResourceManager("INVBTedit.INVedit.Resources", typeof(Data).Assembly);
					...
					if (line[0]=='~') {
						...
						if (split.Length == 5) {
							...
							if (str[0] == '(')
							{
								if (str[str.Length - 1] != ')')
								{
									throw new DataException("Failed to parse column 'DAMAGE' at line " + i + " in file '" + path + "'.");
								}
